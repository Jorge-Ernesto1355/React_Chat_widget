import React from "react";

// Define the type for the component's props
interface AIAssistantIconProps extends React.SVGProps<SVGSVGElement> {
  // We explicitly add a 'color' prop if you want to define its type more strictly
  // Otherwise, React.SVGProps<SVGSVGElement> already includes 'color'
  color?: string; // Make it optional, default will be black if not provided
}

const AIAssistantIcon: React.FC<AIAssistantIconProps> = ({
  color = "#000000", // Default color is black if not specified
  ...props // Capture any other standard SVG props like className, style, etc.
}) => (
  <svg
    width="24px"
    height="24px"
    viewBox="0 0 24 24"
    strokeWidth="1.5"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    // Use the color prop here
    color={color}
    {...props} // Spread any other props onto the svg element
  >
    <path
      d="M6.81818 22L6.81822 19.143C6.51904 16.1656 3.00001 14.5717 3.00001 10.0004C3 5.42914 5.72738 1.94374 11.1819 2.00069C15.1094 2.04169 18.8182 4.28632 18.8182 8.8576L21 12.286C21 14.5717 18.8182 14.5717 18.8182 14.5717C18.8182 14.5717 19.3636 20.2858 14.4545 20.2858L14.4545 22"
      stroke={color} // Apply the color prop here
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M11 12C12.1046 12 13 11.1046 13 10C13 8.89543 12.1046 8 11 8C10.6357 8 10.2942 8.09739 10 8.26756C9.4022 8.61337 9 9.25972 9 10C9 10.7403 9.4022 11.3866 10 11.7324C10.2942 11.9026 10.6357 12 11 12Z"
      stroke={color} // Apply the color prop here
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M11 13C12.6569 13 14 11.6569 14 10C14 8.34315 12.6569 7 11 7C9.34315 7 8 8.34315 8 10C8 11.6569 9.34315 13 11 13Z"
      stroke={color} // Apply the color prop here
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeDasharray="0.3 2"
    />
  </svg>
);

export default AIAssistantIcon;
